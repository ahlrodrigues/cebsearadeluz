...
describe('UserPassesPage', () => {
  it('renders active cycle and actions', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Controle de passes')).toBeInTheDocument())
    expect(screen.getByText('Assistido: Joana Teste')).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /Registrar presença/i })).toBeEnabled()
    expect(screen.getByRole('button', { name: /Exibir QR code/i })).toBeEnabled()
    expect(screen.getAllByRole('cell', { name: 'Presente' }).length).toBeGreaterThan(0)
  })
})
