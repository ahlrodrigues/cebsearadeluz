...
export const getUserQrData = async (userId: number): Promise<UserQrResponse> => {
...
const cleanUpdatePayload = (payload: UpdateUserPayload): UpdateUserPayload => {
  ...
  const assign = <K extends keyof UpdateUserPayload>(
    ...
    if (typeof value === 'string') {
      const normalized = normalize(value)
      if (normalized !== undefined) {
        result[key] = transform ? (transform(normalized) as UpdateUserPayload[K]) : (normalized as UpdateUserPayload[K])
      }
      return
    }
...
export const updateUser = async (userId: number, payload: UpdateUserPayload): Promise<UserResponse> => {
  const response = await apiClient.put<UserResponse>(`/users/${userId}`, cleanUpdatePayload(payload))
  return response.data
}
